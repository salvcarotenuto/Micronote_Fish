...
$toolProject = Join-Path $root 'tools\RemoveRemoteBackupTables\RemoveRemoteBackupTables.csproj'
...
$previous = $env:MICRONOTE_REMOTE_DB_CONNECTION
$env:MICRONOTE_REMOTE_DB_CONNECTION = $remote
try {
    dotnet run --project $toolProject -- $args
}
finally {
    $env:MICRONOTE_REMOTE_DB_CONNECTION = $previous
}
