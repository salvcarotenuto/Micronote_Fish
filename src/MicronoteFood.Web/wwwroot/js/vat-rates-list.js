document.addEventListener("DOMContentLoaded", () => {
  ...
  const grid = page?.querySelector("[data-vat-rate-grid]");
  const table = grid?.querySelector("table");
  const tableBody = table?.querySelector("tbody");
  const sortHeaders = Array.from(table?.querySelectorAll("[data-sort-key]") ?? []);
  const rows = Array.from(table?.querySelectorAll("[data-vat-rate-row]") ?? []);
  const empty = page?.querySelector("[data-vat-rate-empty]");
  const editor = page?.querySelector("[data-vat-rate-editor]");
  const editorFields = editor?.querySelector("[data-vat-rate-editor-fields]");
  const editorIsNew = editor?.querySelector("[data-vat-rate-is-new]");
  const editorCode = editor?.querySelector("[data-vat-rate-code]");
  const editorDescription = editor?.querySelector("[data-vat-rate-description]");
  const editorRate = editor?.querySelector("[data-vat-rate-value]");
  const editorDeduction = editor?.querySelector("[data-vat-rate-deduction]");
  const editorNature = editor?.querySelector("[data-vat-rate-nature]");
  const saveButton = editor?.querySelector("[data-vat-rate-save]");
  const cancelButton = editor?.querySelector("[data-vat-rate-cancel]");
  const actionButtons = Array.from(page?.querySelectorAll("[data-vat-rate-action]") ?? []);
  const deleteForm = page?.querySelector("[data-vat-rate-delete-form]");
  const deleteCode = deleteForm?.querySelector("[data-vat-rate-delete-code]");
  ...
  const setCodeEditable = (editable) => {
    if (!editorCode) {
      return;
    }

    editorCode.readOnly = !editable;
    editorCode.tabIndex = editable ? 0 : -1;
  };
  ...
  const updateEditor = (row) => {
    ...
    if (editorRate) {
      editorRate.value = row.dataset.rate ?? "0";
    }
    if (editorDeduction) {
      editorDeduction.value = row.dataset.deduction ?? "0";
    }
    if (editorNature) {
      editorNature.value = row.dataset.nature ?? "";
    }
  };
  ...
  const requireSelection = () => {
    ...
    if (!row) {
      window.MicronoteMessageBox?.show({
        message: "Selezionare una aliquota IVA.",
        variant: "info"
      });
      return null;
    }
  ...
  grid.addEventListener("keydown", (event) => {
    if (event.target.closest("[data-vat-rate-row]")) {
      return;
    }
  ...
  actionButtons.forEach((button) => {
    button.addEventListener("click", () => {
      const action = button.dataset.vatRateAction;
      if (action === "new") {
        startNew();
        return;
      }
      ...
      if (action === "delete" && row && deleteForm && deleteCode) {
        window.MicronoteMessageBox?.show({
          mode: "confirm",
          variant: "confirm",
          message: `Eliminare l'aliquota IVA ${row.dataset.recordLabel}?`,
