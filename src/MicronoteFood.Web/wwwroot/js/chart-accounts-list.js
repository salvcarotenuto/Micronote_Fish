document.addEventListener("DOMContentLoaded", () => {
  ...
  const grid = page?.querySelector("[data-chart-account-grid]");
  const table = grid?.querySelector("table");
  const tableBody = table?.querySelector("tbody");
  const sortHeaders = Array.from(table?.querySelectorAll("[data-sort-key]") ?? []);
  const rows = Array.from(table?.querySelectorAll("[data-chart-account-row]") ?? []);
  const search = page?.querySelector("[data-chart-account-search]");
  const searchClear = page?.querySelector("[data-chart-account-search-clear]");
  const master = page?.querySelector("[data-chart-account-master]");
  const type = page?.querySelector("[data-chart-account-type]");
  const count = page?.querySelector("[data-chart-account-count]");
  const empty = page?.querySelector("[data-chart-account-empty]");
  const deleteForm = page?.querySelector("[data-chart-account-delete-form]");
  const deleteCode = deleteForm?.querySelector("[data-chart-account-delete-code]");
  ...
  if (!page || !grid || !table || !search || !master || !type) {
    return;
  }
  ...
  const applyFilters = () => {
    ...
    rows.forEach((row) => {
      ...
      const masterMatches =
        masterCode === "" || row.dataset.masterCode === masterCode;
      const typeMatches =
        typeCode === "" || row.dataset.type === typeCode;

      row.hidden = !(textMatches && masterMatches && typeMatches);
      if (!row.hidden) {
        visibleCount += 1;
      }
    });
  ...
  grid.addEventListener("keydown", (event) => {
    if (event.target.closest("[data-chart-account-row]")) {
      return;
    }
  ...
  page.querySelectorAll("[data-chart-account-action]").forEach((button) => {
    button.addEventListener("click", () => {
      const action = button.dataset.chartAccountAction;
      if (action === "edit") {
        editSelected();
        return;
      }
      ...
      if (action === "delete" && row && deleteForm && deleteCode) {
        window.MicronoteMessageBox?.show({
          mode: "confirm",
          variant: "confirm",
          message: `Eliminare il conto ${row.dataset.recordLabel}?`,
  ...
  master.addEventListener("change", applyFilters);
