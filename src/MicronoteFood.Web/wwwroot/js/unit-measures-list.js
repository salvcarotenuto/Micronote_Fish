document.addEventListener("DOMContentLoaded", () => {
  ...
  const grid = page?.querySelector("[data-unit-measure-grid]");
  const table = grid?.querySelector("table");
  const tableBody = table?.querySelector("tbody");
  const sortHeaders = Array.from(table?.querySelectorAll("[data-sort-key]") ?? []);
  const rows = Array.from(table?.querySelectorAll("[data-unit-measure-row]") ?? []);
  const empty = page?.querySelector("[data-unit-measure-empty]");
  const editor = page?.querySelector("[data-unit-measure-editor]");
  const editorFields = editor?.querySelector("[data-unit-measure-editor-fields]");
  const editorCode = editor?.querySelector("[data-unit-measure-code]");
  const editorIsNew = editor?.querySelector("[data-unit-measure-is-new]");
  const editorDescription = editor?.querySelector("[data-unit-measure-description]");
  const saveButton = editor?.querySelector("[data-unit-measure-save]");
  const cancelButton = editor?.querySelector("[data-unit-measure-cancel]");
  const actionButtons = Array.from(page?.querySelectorAll("[data-unit-measure-action]") ?? []);
  const deleteForm = page?.querySelector("[data-unit-measure-delete-form]");
  const deleteCode = deleteForm?.querySelector("[data-unit-measure-delete-code]");
  ...
  const requireSelection = () => {
    ...
    if (!row) {
      window.MicronoteMessageBox?.show({
        message: "Selezionare un'unità di misura.",
        variant: "info"
      });
      return null;
    }
  ...
  grid.addEventListener("keydown", (event) => {
    if (event.target.closest("[data-unit-measure-row]")) {
      return;
    }
  ...
  page.querySelectorAll("[data-unit-measure-action]").forEach((button) => {
    button.addEventListener("click", () => {
      const action = button.dataset.unitMeasureAction;
      if (action === "new") {
        startNew();
        return;
      }
      ...
      if (action === "delete" && row && deleteForm && deleteCode) {
        window.MicronoteMessageBox?.show({
          mode: "confirm",
          variant: "confirm",
          message: `Eliminare l'unità di misura ${row.dataset.recordLabel}?`,
