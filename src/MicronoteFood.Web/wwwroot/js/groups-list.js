document.addEventListener("DOMContentLoaded", () => {
  ...
  const grid = page?.querySelector("[data-group-grid]");
  const table = grid?.querySelector("table");
  const tableBody = table?.querySelector("tbody");
  const sortHeaders = Array.from(table?.querySelectorAll("[data-sort-key]") ?? []);
  const rows = Array.from(table?.querySelectorAll("[data-group-row]") ?? []);
  const empty = page?.querySelector("[data-group-empty]");
  const editor = page?.querySelector("[data-group-editor]");
  const editorFields = editor?.querySelector("[data-group-editor-fields]");
  const editorCode = editor?.querySelector("[data-group-code]");
  const editorCodeDisplay = editor?.querySelector("[data-group-code-display]");
  const editorDescription = editor?.querySelector("[data-group-description]");
  const saveButton = editor?.querySelector("[data-group-save]");
  const cancelButton = editor?.querySelector("[data-group-cancel]");
  const actionButtons = Array.from(page?.querySelectorAll("[data-group-action]") ?? []);
  const deleteForm = page?.querySelector("[data-group-delete-form]");
  const deleteCode = deleteForm?.querySelector("[data-group-delete-code]");
  ...
  grid.addEventListener("keydown", (event) => {
    if (event.target.closest("[data-group-row]")) {
      return;
    }
  ...
  page.querySelectorAll("[data-group-action]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (action === "delete" && row && deleteForm && deleteCode) {
        window.MicronoteMessageBox?.show({
          mode: "confirm",
          variant: "confirm",
          message: `Eliminare il gruppo ${row.dataset.recordLabel}?`,
