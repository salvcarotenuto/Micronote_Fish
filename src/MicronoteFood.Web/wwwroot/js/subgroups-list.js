document.addEventListener("DOMContentLoaded", () => {
  ...
  const grid = page?.querySelector("[data-subgroup-grid]");
  const table = grid?.querySelector("table");
  const tableBody = table?.querySelector("tbody");
  const sortHeaders = Array.from(table?.querySelectorAll("[data-sort-key]") ?? []);
  const rows = Array.from(table?.querySelectorAll("[data-subgroup-row]") ?? []);
  const empty = page?.querySelector("[data-subgroup-empty]");
  const editor = page?.querySelector("[data-subgroup-editor]");
  const editorFields = editor?.querySelector("[data-subgroup-editor-fields]");
  const editorCode = editor?.querySelector("[data-subgroup-code]");
  const editorCodeDisplay = editor?.querySelector("[data-subgroup-code-display]");
  const editorDescription = editor?.querySelector("[data-subgroup-description]");
  const saveButton = editor?.querySelector("[data-subgroup-save]");
  const cancelButton = editor?.querySelector("[data-subgroup-cancel]");
  const actionButtons = Array.from(page?.querySelectorAll("[data-subgroup-action]") ?? []);
  const deleteForm = page?.querySelector("[data-subgroup-delete-form]");
  const deleteCode = deleteForm?.querySelector("[data-subgroup-delete-code]");
  ...
  const requireSelection = () => {
    ...
    if (!row) {
      window.MicronoteMessageBox?.show({
        message: "Selezionare un sottogruppo.",
        variant: "info"
      });
      return null;
    }
  ...
  grid.addEventListener("keydown", (event) => {
    if (event.target.closest("[data-subgroup-row]")) {
      return;
    }
  ...
  page.querySelectorAll("[data-subgroup-action]").forEach((button) => {
    button.addEventListener("click", () => {
      const action = button.dataset.subgroupAction;
      if (action === "new") {
        startNew();
        return;
      }
      ...
      if (action === "delete" && row && deleteForm && deleteCode) {
        window.MicronoteMessageBox?.show({
          mode: "confirm",
          variant: "confirm",
          message: `Eliminare il sottogruppo ${row.dataset.recordLabel}?`,
