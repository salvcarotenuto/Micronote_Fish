document.addEventListener("DOMContentLoaded", () => {
  ...
  const grid = page?.querySelector("[data-store-grid]");
  const table = grid?.querySelector("table");
  const tableBody = table?.querySelector("tbody");
  const sortHeaders = Array.from(table?.querySelectorAll("[data-sort-key]") ?? []);
  const rows = Array.from(table?.querySelectorAll("[data-store-row]") ?? []);
  const search = page?.querySelector("[data-store-search]");
  const searchClear = page?.querySelector("[data-store-search-clear]");
  const count = page?.querySelector("[data-store-count]");
  const empty = page?.querySelector("[data-store-empty]");
  const deleteForm = page?.querySelector("[data-store-delete-form]");
  const deleteCode = deleteForm?.querySelector("[data-store-delete-code]");
  ...
  const requireSelection = () => {
    ...
    if (!row) {
      window.MicronoteMessageBox?.show({
        message: "Selezionare un punto vendita.",
        variant: "info"
      });
      return null;
    }
  ...
  grid.addEventListener("keydown", (event) => {
    if (event.target.closest("[data-store-row]")) {
      return;
    }
  ...
  page.querySelectorAll("[data-store-action]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (action === "delete" && row && deleteForm && deleteCode) {
        window.MicronoteMessageBox?.show({
          mode: "confirm",
          variant: "confirm",
          message: `Eliminare il punto vendita ${row.dataset.recordLabel}?`,
