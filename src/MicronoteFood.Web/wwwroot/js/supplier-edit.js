(function () {
  ...
  const saveButton = document.querySelector("[data-supplier-save]");

  if (!form || !saveButton) {
    return;
  }

  const field = (name) => form.querySelector(`[name="Supplier.${name}"]`);
  ...
  taxCodeField?.addEventListener("input", () => {
    const start = taxCodeField.selectionStart;
    taxCodeField.value = normalizeFiscalValue(taxCodeField.value).slice(0, 16);
    taxCodeField.setSelectionRange(start, start);
  });
