document.addEventListener("DOMContentLoaded", () => {
  ...
  const grid = page?.querySelector("[data-ledger-master-grid]");
  const table = grid?.querySelector("table");
  const tableBody = table?.querySelector("tbody");
  const sortHeaders = Array.from(table?.querySelectorAll("[data-sort-key]") ?? []);
  const rows = Array.from(table?.querySelectorAll("[data-ledger-master-row]") ?? []);
  const empty = page?.querySelector("[data-ledger-master-empty]");
  const editor = page?.querySelector("[data-ledger-master-editor]");
  const editorFields = editor?.querySelector("[data-ledger-master-editor-fields]");
  const editorIsNew = editor?.querySelector("[data-ledger-master-is-new]");
  const editorCode = editor?.querySelector("[data-ledger-master-code]");
  const editorDescription = editor?.querySelector("[data-ledger-master-description]");
  const editorType = editor?.querySelector("[data-ledger-master-type]");
  const editorLocked = editor?.querySelector("[data-ledger-master-locked]");
  const saveButton = editor?.querySelector("[data-ledger-master-save]");
  const cancelButton = editor?.querySelector("[data-ledger-master-cancel]");
  const actionButtons = Array.from(page?.querySelectorAll("[data-ledger-master-action]") ?? []);
  const deleteForm = page?.querySelector("[data-ledger-master-delete-form]");
  const deleteCode = deleteForm?.querySelector("[data-ledger-master-delete-code]");
  ...
  const codeText = (code) => code ? String(code).padStart(3, "0") : "";
  let currentSortKey = "";
  ...
  const updateEditor = (row) => {
    if (!editorCode || !editorDescription || !row) {
      return;
    }
    ...
    editorCode.value = codeText(row.dataset.code);
    editorCode.readOnly = true;
    editorDescription.value = row.dataset.description ?? "";
    if (editorType) {
      editorType.value = row.dataset.type ?? "";
    }
    if (editorLocked) {
      editorLocked.checked = row.dataset.locked === "true";
    }
  };
  ...
  const startNew = () => {
    ...
    if (editorCode) {
      editorCode.value = page.dataset.nextLedgerMasterCode || "";
      editorCode.readOnly = true;
    }
  ...
  grid.addEventListener("keydown", (event) => {
    if (event.target.closest("[data-ledger-master-row]")) {
      return;
    }
  ...
  actionButtons.forEach((button) => {
    button.addEventListener("click", () => {
      const action = button.dataset.ledgerMasterAction;
      if (action === "new") {
        startNew();
        return;
      }
      ...
      if (action === "delete" && row && deleteForm && deleteCode) {
        window.MicronoteMessageBox?.show({
          mode: "confirm",
          variant: "confirm",
          message: `Eliminare il mastro ${row.dataset.recordLabel}?`,
  ...
  editorCode?.addEventListener("blur", () => {
    editorCode.value = codeText(editorCode.value.trim());
  });
