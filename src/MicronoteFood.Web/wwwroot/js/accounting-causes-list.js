document.addEventListener("DOMContentLoaded", () => {
  ...
  const grid = page?.querySelector("[data-accounting-cause-grid]");
  const table = grid?.querySelector("table");
  const tableBody = table?.querySelector("tbody");
  const sortHeaders = Array.from(table?.querySelectorAll("[data-sort-key]") ?? []);
  const rows = Array.from(table?.querySelectorAll("[data-accounting-cause-row]") ?? []);
  const search = page?.querySelector("[data-accounting-cause-search]");
  const searchClear = page?.querySelector("[data-accounting-cause-search-clear]");
  const type = page?.querySelector("[data-accounting-cause-type]");
  const count = page?.querySelector("[data-accounting-cause-count]");
  const empty = page?.querySelector("[data-accounting-cause-empty]");
  const deleteForm = page?.querySelector("[data-accounting-cause-delete-form]");
  const deleteCode = deleteForm?.querySelector("[data-accounting-cause-delete-code]");
  ...
  if (!page || !grid || !table || !search || !type) {
    return;
  }
  ...
  const applyFilters = () => {
    ...
    rows.forEach((row) => {
      ...
      const typeMatches =
        typeCode === "" || row.dataset.movementType === typeCode;

      row.hidden = !(textMatches && typeMatches);
      if (!row.hidden) {
        visibleCount += 1;
      }
    });
  ...
  const requireSelection = () => {
    ...
    if (!row) {
      window.MicronoteMessageBox?.show({
        message: "Selezionare una causale.",
        variant: "info"
      });
      return null;
    }
  ...
  const duplicateSelected = () => {
    const row = requireSelection();
    if (row?.dataset.duplicateUrl) {
      window.location.href = row.dataset.duplicateUrl;
    }
  };
  ...
  grid.addEventListener("keydown", (event) => {
    if (event.target.closest("[data-accounting-cause-row]")) {
      return;
    }
  ...
  page.querySelectorAll("[data-accounting-cause-action]").forEach((button) => {
    button.addEventListener("click", () => {
      const action = button.dataset.accountingCauseAction;
      if (action === "edit") {
        editSelected();
        return;
      }
