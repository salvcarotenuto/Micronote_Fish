document.addEventListener("DOMContentLoaded", () => {
  ...
  const rows = Array.from(table?.querySelectorAll("[data-supplier-row]") ?? []);
  const search = page?.querySelector("[data-supplier-search]");
  const searchClear = page?.querySelector("[data-supplier-search-clear]");
  const account = page?.querySelector("[data-supplier-account]");
  const count = page?.querySelector("[data-supplier-count]");
  const empty = page?.querySelector("[data-supplier-empty]");
  const deleteForm = page?.querySelector("[data-supplier-delete-form]");
  const deleteCode = deleteForm?.querySelector("[data-supplier-delete-code]");
  const preview = page?.querySelector("[data-supplier-print-preview]");
  const previewDocument = preview?.querySelector("[data-supplier-preview-document]");
  const previewSummary = preview?.querySelector("[data-supplier-print-summary]");
  const previewZoomLabel = preview?.querySelector("[data-supplier-preview-zoom-label]");
  ...
  const visibleRows = () =>
    Array.from(tableBody?.querySelectorAll("[data-supplier-row]") ?? [])
  ...
  const requireSelection = () => {
    ...
    if (!row) {
      window.MicronoteMessageBox?.show({
        message: "Selezionare un fornitore.",
        variant: "info"
      });
      return null;
    }
  ...
  const buildPrintPreview = () => {
    ...
    if (sourceRows.length === 0) {
      window.MicronoteMessageBox?.show({
        message: "Non ci sono fornitori da stampare con i filtri correnti.",
        variant: "info"
      });
      return;
    }
    ...
    for (let pageIndex = 0; pageIndex < pageCount; pageIndex += 1) {
      ...
      heading.innerHTML = `
        <div><strong>Lista fornitori</strong><span>Elaborazione del ${generatedAt} · ${sourceRows.length} record</span></div>
    ...
    preview.querySelector("[data-supplier-preview-close]")?.focus();
  };

  const downloadPdf = async () => {
    const sourceRows = printableRows();
    const token = page.querySelector('input[name="__RequestVerificationToken"]')?.value;
    const button = preview?.querySelector("[data-supplier-preview-pdf]");
  ...
  grid.addEventListener("keydown", (event) => {
    if (event.target.closest("[data-supplier-row]")) {
      return;
    }
  ...
  page.querySelectorAll("[data-supplier-action]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (action === "delete" && row && deleteForm && deleteCode) {
        window.MicronoteMessageBox?.show({
          mode: "confirm",
          variant: "confirm",
          message: `Eliminare il fornitore ${row.dataset.recordLabel}?`,
  ...
  preview?.querySelector("[data-supplier-preview-close]")?.addEventListener("click", closePrintPreview);
  preview?.querySelector("[data-supplier-preview-zoom-out]")?.addEventListener("click", () => {
    previewZoom = Math.max(0.5, previewZoom - 0.1);
    updatePreviewZoom();
  });
  preview?.querySelector("[data-supplier-preview-zoom-in]")?.addEventListener("click", () => {
    previewZoom = Math.min(1.8, previewZoom + 0.1);
    updatePreviewZoom();
  });
  preview?.querySelector("[data-supplier-preview-print]")?.addEventListener("click", () => {
    document.body.classList.add("is-printing-supplier-report");
    window.print();
  });
  preview?.querySelector("[data-supplier-preview-pdf]")?.addEventListener("click", downloadPdf);
