document.addEventListener("DOMContentLoaded", () => {
  ...
  const grid = page?.querySelector("[data-category-grid]");
  const table = grid?.querySelector("table");
  const tableBody = table?.querySelector("tbody");
  const sortHeaders = Array.from(table?.querySelectorAll("[data-sort-key]") ?? []);
  const rows = Array.from(table?.querySelectorAll("[data-category-row]") ?? []);
  const empty = page?.querySelector("[data-category-empty]");
  const editor = page?.querySelector("[data-category-editor]");
  const editorFields = editor?.querySelector("[data-category-editor-fields]");
  const editorCode = editor?.querySelector("[data-category-code]");
  const editorCodeDisplay = editor?.querySelector("[data-category-code-display]");
  const editorDescription = editor?.querySelector("[data-category-description]");
  const saveButton = editor?.querySelector("[data-category-save]");
  const cancelButton = editor?.querySelector("[data-category-cancel]");
  const actionButtons = Array.from(page?.querySelectorAll("[data-category-action]") ?? []);
  const deleteForm = page?.querySelector("[data-category-delete-form]");
  const deleteCode = deleteForm?.querySelector("[data-category-delete-code]");
  ...
  grid.addEventListener("keydown", (event) => {
    if (event.target.closest("[data-category-row]")) {
      return;
    }
  ...
  page.querySelectorAll("[data-category-action]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (action === "delete" && row && deleteForm && deleteCode) {
        window.MicronoteMessageBox?.show({
          mode: "confirm",
          variant: "confirm",
          message: `Eliminare la categoria ${row.dataset.recordLabel}?`,
