...
public sealed class CustomerListPdfService
{
    static CustomerListPdfService()
    {
        GlobalFontSettings.UseWindowsFontsUnderWindows = true;
    }

    private static readonly (string Title, double Width, Func<CustomerListItem, string> Value)[] Columns =
    [
    ...
    public byte[] Create(IReadOnlyList<CustomerListItem> customers, DateTime generatedAt)
    {
        using var document = new PdfDocument();
        document.Info.Title = "Lista clienti";
        document.Info.Subject = "Elenco clienti filtrato e ordinato dall'utente";
        ...
        var pageCount = Math.Max(1, (int)Math.Ceiling(customers.Count / (double)rowsPerPage));
        ...
        for (var pageIndex = 0; pageIndex < pageCount; pageIndex++)
        {
            ...
            graphics.DrawString("Lista clienti", titleFont, XBrushes.Black,
                new XRect(Margin, Margin, width - (Margin * 2), 24), XStringFormats.TopLeft);
            graphics.DrawString(
                $"Elaborazione del {generatedAt:dd/MM/yyyy HH:mm}  •  {customers.Count} record",
            ...
            var pageRows = customers.Skip(pageIndex * rowsPerPage).Take(rowsPerPage).ToArray();

            for (var rowIndex = 0; rowIndex < pageRows.Length; rowIndex++)
            {
                var customer = pageRows[rowIndex];
                DrawRow(graphics, customer, top + HeaderHeight + (rowIndex * RowHeight),
                    rowIndex % 2 == 1, customer.IsActive ? rowFont : inactiveFont);
            }
    ...
    private static void DrawRow(XGraphics graphics, CustomerListItem customer, double y,
        bool alternate, XFont font)
    {
        var x = Margin;
        var background = !customer.IsActive ? XColor.FromArgb(245, 231, 231)
            : alternate ? XColor.FromArgb(239, 244, 250) : XColors.White;
        ...
        foreach (var column in Columns)
        {
            ...
            DrawCellText(graphics, column.Value(customer), font, XBrushes.Black, rectangle, 3);
            x += column.Width;
        }
    }
